...
let
  ...
  cfg = config.nixdisplay.monitors;
    ...
    options = edidOptions // {
      ...
      nativeMode = mkOption {
        ...
        description = ''
          ...
          mode says so itself (`nixdisplay.layouts.<name>.outputs.*.mode`). It is recorded here
  ...
  identities = lib.concatLists (lib.mapAttrsToList
    (slug: m:
      [{ where = "nixdisplay.monitors.${slug}"; inherit (m) identifier; }]
      ++ lib.imap0
        (i: a: {
          where = "nixdisplay.monitors.${slug}.aliases[${toString i}]";
  ...
  # layouts defensively (`config.nixdisplay ? layouts`) rather than the other way round also keeps
  ...
  layouts = if config.nixdisplay ? layouts then config.nixdisplay.layouts else { };
...
in
{
  options.nixdisplay.monitors = mkOption {
    ...
    description = ''
      ...
      (`nixdisplay.layouts`). Empty by default — a host that only ever addresses connectors
  ...
  config.assertions =
    ...
    lib.mapAttrsToList
      (identifier: entries: {
        assertion = false;
        message = ''
          nixdisplay.monitors: ${toString (lib.length entries)} declarations resolve to the SAME
    ...
    ++ map
      (r: {
        assertion = false;
        message = ''
          nixdisplay.layouts.${r.layout}.outputs[${toString r.index}] addresses the monitor
