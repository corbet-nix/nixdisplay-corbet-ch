...
let
  ...
  monitorsComposed = config.nixdisplay ? monitors;
  monitors = if monitorsComposed then config.nixdisplay.monitors else { };
  ...
  # lets each message say `nixdisplay.layouts.docked.outputs[2]` rather than "an output".
  ...
  addressOf = e: "nixdisplay.layouts.${e.layout}.outputs[${toString e.index}] (${selectorOf e.output})";
  ...
  outputModule = {
    options = {
      monitor = mkOption {
        ...
        example = "u4323qe";
        description = ''
          A slug into `nixdisplay.monitors` -- the panel this entry configures, wherever it is
        ...
        description = ''
          ...
          two ways to address an output, and the only one available in three real cases:
            ...
            - a panel whose EDID carries no usable serial, so its identity triple is shared with
              every other unit of its model (`nixdisplay.monitors.<name>.identifiable = false`);
      ...
      match = mkOption {
        ...
        description = ''
          ...
          `nixdisplay.monitors.<name>.identifier` -- or `"connector"`, the connector name.
      ...
      mode = mkOption {
        ...
        description = ''
          ...
          `nixdisplay.monitors.<name>.nativeMode`, and if that is null too the output's rectangle
  ...
  layoutModule = {
    options = {
      ...
      outputs = mkOption {
        ...
        default = [ ];
        description = ''
          The outputs this layout arranges, each addressed either by a `nixdisplay.monitors` slug
...
in
{
  options.nixdisplay.layouts = mkOption {
  ...
  config.assertions =
    ...
    map
      (e: {
        assertion = false;
        message = ''
          nixdisplay.layouts.${e.layout}.outputs[${toString e.index}] sets
          ${if e.output.monitor != null then "BOTH `monitor` and `connector`" else "NEITHER `monitor` nor `connector`"}.
          Exactly one must be set: an output is addressed either by a `nixdisplay.monitors` slug
    ...
    ++ map
      (e: {
        ...
        message = ''
          ...
          `"<make> <model> <serial>"` triple from `nixdisplay.monitors.<name>.identifier`, and
    ...
    ++ map
      (e: {
        ...
        message = ''
          ...
          `nixdisplay.monitors` slug cannot supply one, because the registry is fleet-wide and a
    ...
    ++ lib.optionals monitorsComposed (map
      (e: {
        ...
        message = ''
          ...
          `nixdisplay.monitors` does not declare. Declared slugs:
    ...
    ++ map
      (pr: {
        assertion = false;
        message = ''
          nixdisplay.layouts.${pr.a.layout}: two enabled outputs overlap.
  ...
  config.warnings = map
    (e: ''
      ...
      no `mode`, carries no `modeline`, and ${if e.output.monitor == null then "addresses a connector, so there is no registry entry to take a `nativeMode` from" else "its `nixdisplay.monitors` entry records no `nativeMode`"}.
      ...
      Record `nixdisplay.monitors.<name>.nativeMode`, or pin a `mode` here, to close the gap.
    '')
    unsizedPlaced;
}
