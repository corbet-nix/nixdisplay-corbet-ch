...
# `nixdisplay.layouts` is translated by each compositor module (nixniri, nixscroll) into that
...
# for the same monitors -- in practice by leaving the compositor's own `layout` (its nixscroll/nixniri option)
...
{ config, lib, ... }:
let
  cfg = config.nixdisplay.dynamicOutputs;

  layouts = config.nixdisplay.layouts or { };
  monitors = config.nixdisplay.monitors or { };
  ...
  criteriaOf = o:
    if o.match == "connector" then o.connector
      ...
      "nixdisplay.dynamicOutputs: layout output references monitor '${o.monitor}', which is not in nixdisplay.monitors."));
  ...
  # (wayland.xml: "90 degrees counter-clockwise"), which is the same direction `nixdisplay.layouts`
  ...
  renderProfile = name:
    let l = layouts.${name} or (throw
      "nixdisplay.dynamicOutputs.profiles names '${name}', which is not a declared nixdisplay.layouts entry.");
    in
...
in
{
  options.nixdisplay.dynamicOutputs = {
    enable = lib.mkEnableOption ''
      kanshi profile generation from `nixdisplay.layouts`, for hosts whose correct output
      ...
      `layout` (its nixscroll/nixniri option) null. See this file's header: the two race, last writer wins, and the
    ...
    profiles = lib.mkOption {
      ...
      description = ''
        Names of `nixdisplay.layouts` entries to emit as kanshi profiles.
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "nixdisplay.dynamicOutputs is enabled but names no profiles, which would emit a config with no profile in it -- kanshi treats a missing/empty config as a fatal error (main.c:720), so the session would get no output management at all rather than a harmless no-op.";
      }
      {
        assertion = lib.all (m: (monitors.${m}.aliases or [ ]) == [ ]) referencedMonitors;
        message = ''
          nixdisplay.dynamicOutputs: a profile addresses a monitor that declares `aliases`, which
    ...
    xdg.configFile."kanshi/config".text = ''
      # Generated by nixdisplay (nixdisplay.dynamicOutputs) from nixdisplay.layouts.
    ...
    # A STANDALONE graphical-session user service. nixdisplay owns its daemon end to end rather than
    # routing it through nixdesktop's `session.services` layer: writing into a sibling repo's option
    # namespace would make this module fail to evaluate on any host that has not composed nixdesktop,
    # coupling the two exactly where this repo's split exists to decouple them. That layer earns its
    # keep giving a WEB of interdependent session components safe ordering and restart-backoff (its
    # own checks/session-ordering-cycles and session-restart-backoff prove the failure modes it
    # prevents); kanshi is a dependency-free LEAF -- nothing is ordered after it, nothing waits on it
    # -- so not one of those hazards can arise, and a plain user unit is both sufficient and more
    # honest. The wiring reproduces the defaults that layer would have applied to a service declaring
    # only a command: PartOf/After/WantedBy = graphical-session.target.
    #
    ...
    # own `/bin/sh -c` child.
    #
    # NO `ExecReload`: the obvious `kanshictl reload` is only built when libvali was present at
    # compile time (meson.build wraps it in `if vali.found()`), so a unit asserting it would break on
    # distro builds without it. kanshi rereads its config on SIGHUP regardless, which
    # `systemctl --user kill -s HUP kanshi` already does with no unit support.
    #
    # `ExecStart = "kanshi"` names no absolute path -- like every config module in this family, the
    # daemon is supplied by whatever puts binaries on the box (a platform backend such as nixarch, or
    # NixOS `environment.systemPackages`), never installed here. systemd resolves the bare name
    # against its own compiled-in exec search path (which includes the /usr/bin the distro package
    # lands in), not the manager's imported $PATH.
    systemd.user.services.kanshi = {
      Unit = {
        Description = "Dynamic output configuration (kanshi profiles from nixdisplay.layouts)";
