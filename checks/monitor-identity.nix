...
let
  ...
  monitorsOnly = defs: evalWith [ ../modules/monitors.nix { nixdisplay.monitors = defs; } ];
  ...
  bothTables = mon: lay: evalWith [
    ...
    ../modules/layouts.nix
    { nixdisplay.monitors = mon; nixdisplay.layouts = lay; }
  ];

  mon = slug: cfg: (monitorsOnly { ${slug} = cfg; }).nixdisplay.monitors.${slug};
  ...
  results = {
    ...
    "the collision message names the identity and both declarations" =
      ...
      && lib.hasInfix "nixdisplay.monitors.a" m
      && lib.hasInfix "nixdisplay.monitors.b" m;
    ...
    "a three-way collision is reported once, naming all four declarations" =
      ...
      && lib.hasInfix "nixdisplay.monitors.c.aliases[0]" (lib.head msgs);
    ...
    "that message names the layout entry and the shared identity" =
      ...
      lib.hasInfix "nixdisplay.layouts.desk.outputs[0]" m
